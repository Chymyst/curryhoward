...
When the macros `allOfType` and `ofType` are used to create a function type such as `A ⇒ B ⇒ C`, the returned value is actually of a special subclass of `Function1`, `Function2`, or `Function3`, which carries not only the function's compiled code but also the STLC term corresponding to the function's code.
...
We could write unit tests (e.g. using `scalacheck`) for verifying this law on specific values of `x` and `y`, but this will not give the same level of assurance as a symbolic mathematical proof.
...
res9: Seq[io.chymyst.ch.TermExpr] = List(y$2, x$1)
```
...
res10: Seq[io.chymyst.ch.TermExpr] = List(((\((a:<c>Int) ⇒ (b:<c>Int) ⇒ a) x$1) y$2))
```
